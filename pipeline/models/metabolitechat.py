...
class MetaboliteChat(BaseModel):
    ...
    def encode_img(self, inputs, device, do_proj=True):
        ...
        if "gnn" in self.encoder_names:
            ...
            feat = graph_feat
            inputs["feat"] = feat
            inputs["graph_feat"] = feat # [batch_size, 1, 300]
        if "image_mol" in self.encoder_names:
            image = inputs['image']
            device = image.device
            if self.low_resource:
            ...
            inputs["feat"] = feat
            inputs["image_feat"] = feat # [batch_size, 1, 512]
    ...
    def proj_feat(self, features, device):
        ...
        if self.feat_dims is not None:
            ...
            img_embeds = torch.cat(feats, dim=1)  # (batchsize, 2, 5120)
            atts_img = torch.ones(img_embeds.size()[:-1], dtype=torch.long).to(device) # (4, 2)?
    ...
    def prompt_wrap(self, img_embeds, atts_img, prompts):
        if prompts:
            ...
            p_before_tokens = self.llama_tokenizer(
                p_before, padding="longest", return_tensors="pt", add_special_tokens=False).to(img_embeds.device) # (batch_size, length of longest token sequence) in the form of dictionary id.
            p_after_tokens = self.llama_tokenizer(
                p_after, padding="longest", return_tensors="pt", add_special_tokens=False).to(img_embeds.device)  # similar to line 294
            p_before_embeds = self.llama_embed_tokens(p_before_tokens.input_ids)#.expand(batch_size, -1, -1)      # (batch_size, length, 5120)
            ...
            wrapped_img_embeds = torch.cat([p_before_embeds, img_embeds, p_after_embeds], dim=1)                  # (batch_size, input_token_length, 5120)
            wrapped_atts_img = atts_img[:, :1].expand(-1, wrapped_img_embeds.shape[1])                            # (batch_size, input_token_length) all 1 vector.
    ...
    def forward(self, samples):
        ...
        img_embeds, atts_img = self.encode_img(samples, device) # (n, 2, 5120), (n, 2)
        ...
        ).to(device)                                               # (batch_size, length of longest token sequence) in the form of dictionary id. for example (4, 400)

        targets = to_regress_tokens.input_ids.masked_fill(                                    # mask all padded tokens
        ...
        )                                                          # should be all -100 in shape of (bs, length of input tokens + 1)
        ...
        attention_mask = torch.cat([atts_bos, atts_img, to_regress_tokens.attention_mask], dim=1)      # all 1 vector ?
